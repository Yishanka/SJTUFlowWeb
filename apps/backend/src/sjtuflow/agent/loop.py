...
- For simple 签到/点名 questions, distinguish the target: "我有没有签到/签到记录/考勤状态" means inspect Canvas course data and LTI sign-in pages; "老师有没有提到签到/点名" means search local transcripts. When the course is named but no sign-in URL is given, use Canvas token tools to identify the course and module/external-tool links first; use browser external-tool readers only for pages the user can access, and ask for the Canvas `external_tools` URL if the sign-in entry is only visible in the course navigation.
