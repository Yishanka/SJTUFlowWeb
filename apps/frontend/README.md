...
当前实现采用零前端依赖的静态单页应用，直接调用 FastAPI 后端的
`/api/*` 接口。这样便于课程项目演示，也避免因为安装前端依赖影响运行。

仓库根目录启动后端时，后端会直接托管本目录的 `index.html`、`app.js`、
`styles.css` 和 `assets/`。`dist/` 存在时优先托管构建产物。
...
- 媒体转写：本地媒体、Canvas 课程视频页面转写、Canvas external_tools 候选页查找
